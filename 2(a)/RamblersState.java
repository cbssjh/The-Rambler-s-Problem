...
public class RamblersState extends SearchState{
	...
	// constructor
	 public RamblersState (Coords coordinates, int c, int rc){
		 	cY = coordinates.gety();
		 	cX = coordinates.getx();
	        localCost = c;
	        estRemCost = rc;
	...
	// goalp
	 ...
	 public int mDistance(RamblersSearch r, int nY, int nX) {
		 int gX = r.getGoalX();
		 int gY = r.getGoalY();
		 int dis = Math.abs(nY-gY) + Math.abs(nX-gX);
		 return dis;
	 }
	 
	 public int eDistance(RamblersSearch r, int nY, int nX) {
		 int gX = r.getGoalX();
		 int gY = r.getGoalY();
		 int dis = (int) Math.pow((Math.pow(nY-gY, 2)+(Math.pow(nX-gX, 2))), 0.5);
		 return dis;
	 }
	 
	 // function to calculate the height diff
	 public int difHeight(TerrainMap nMap, RamblersSearch r, int nY, int nX) {
		 int goalHeight = nMap.getTmap()[r.getGoalY()][r.getGoalX()];
		 int height = nMap.getTmap()[nY][nX];
		 int dif = Math.abs(goalHeight - height);
		 return dif;
	 }
	 
	 // function to calculate local cost
	...
	// getSuccessors
	 public ArrayList<SearchState> getSuccessors (Search searcher){
		 ...
		 for (Coords coordinates : Coordinates) {
			 ...
			 succs.add(new RamblersState(coordinates, cost, difHeight(nMap, rsearcher, y, x))); 
		    //succs.add(new RamblersState(coordinates, cost, mDistance(rsearcher, y, x))); // for Manhatten distance
		    //succs.add(new RamblersState(coordinates, cost, eDistance(rsearcher, y, x))); // for Euclidean distance
		 }
		 return succs;
	 } 
